...
def fail(reason):
    """ Returns a bencoded failure message """
    failcode = 900
    if isinstance(reason, ( int, long )):
        ...
        reason = str(reason)
    raise bottle.HTTPResponse("d14:failure reason%d:%s12:failure codei%dee" % (len(reason), reason, failcode), header={'Content-type': 'text/plain'})
